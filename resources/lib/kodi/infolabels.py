...
from resources.lib.common.cache_utils import CACHE_BOOKMARKS, CACHE_INFOLABELS, generate_identifier, CACHE_ARTINFO
...
def get_info(videoid, item, raw_data):
    ...
    cache_identifier = generate_identifier(videoid.value + '_' + g.LOCAL_DB.get_profile_config('language', ''))
    try:
...
def get_art(videoid, item, raw_data=None):
    """Get art infolabels"""
    cache_identifier = generate_identifier(videoid.value)
    try:
